...
This is the most likely kind of contribution right now — twelve engines
...
10. Document it in `README.md`'s `## Details` `### Machine translation
    ...
    `website/docs/cli.md`'s "Twelve engines are built in: ..." and
    `website/docs/index.md`'s "all twelve translation engines (...)" intro
...
One exception: the "Latest packages" table in `README.md`'s `## Details`
`### Distro packages` (between the `<!-- PACKAGES_TABLE_START/END -->`
markers) is regenerated by `release.yml` from a release's actual assets
(`scripts/update-packages-table.py`) — never hand-edit the rows inside
those markers, that edit is overwritten by the next release. Everything
else in that section is a normal, manually-maintained doc.
